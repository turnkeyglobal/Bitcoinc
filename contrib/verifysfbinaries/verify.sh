...
###   This script attempts to download the signature file SHA256SUMS.asc from minmontcoin.org
...
WORKINGDIR="/tmp/minmontcoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://minmontcoin.org/bin/"
VERSIONPREFIX="minmontcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'minmontcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Investcoin Core binary release signing key installed?"
   fi
