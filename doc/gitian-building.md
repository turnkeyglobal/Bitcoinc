...
*Setup instructions for a Gitian build of Minmontcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Minmontcoin
...
- [Building Minmontcoin](#building-minmontcoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-minmontcoin-host/README.md).
...
Clone the git repositories for minmontcoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/minmontcoin/minmontcoin
```
...
in the minmontcoin repository under 'Fetch and build inputs' to install sources which require
...
To build Minmontcoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the minmontcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/minmontcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/minmontcoin/minmontcoin
...
For example:
```bash
URL=https://github.com/laanwj/minmontcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit minmontcoin=${COMMIT} --url minmontcoin=${URL} ../minmontcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit minmontcoin=${COMMIT} --url minmontcoin=${URL} ../minmontcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit minmontcoin=${COMMIT} --url minmontcoin=${URL} ../minmontcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the minmontcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../minmontcoin/contrib/gitian-descriptors/*|sort|uniq )
...
INVPATH=/some/root/path/minmontcoin.git
SIGPATH=/some/root/path/minmontcoin-detached-sigs.git

./bin/gbuild --url minmontcoin=${INVPATH},signature=${SIGPATH} ../minmontcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/minmontcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/minmontcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/minmontcoin-osx-build.assert
```
...
[minmontcoin/gitian.sigs](https://github.com/minmontcoin/gitian.sigs/) repository, or if that's not possible create a pull
